...
from virality_analyzer import (
    ViralityAnalyzer, ContentData, ContentMetrics, 
    ContentType, Platform, CausalAnalyzer
)
...
def generate_synthetic_dataset(n_samples=200):
    ...
    content_data_list = []
    platforms = [Platform.TWITTER, Platform.INSTAGRAM, Platform.YOUTUBE, Platform.TIKTOK]
    ...
    for i in range(n_samples):
        ...
        likes = int(base_impressions * np.random.uniform(0.01, 0.1) * engagement_multiplier)
        ...
        topics = ["AI breakthrough", "life hack", "travel destination", "recipe", "workout"]
        ...
        if "{topic}" in template:
            text = template.format(topic=np.random.choice(topics))
        elif "{event}" in template:
            text = template.format(event=np.random.choice(events))
        elif "{news}" in template:
            text = template.format(news=np.random.choice(news))
        elif "{tip}" in template:
            text = template.format(tip=np.random.choice(tips))
        elif "{process}" in template:
            text = template.format(process=np.random.choice(processes))
        else:
            text = template
        ...
        content_data = ContentData(
            ...
            metrics=ContentMetrics(
                ...
                saves=int(likes * np.random.uniform(0.02, 0.08)) if np.random.random() > 0.5 else None
            ),
...
def demonstrate_causal_analysis():
    ...
    try:
        ...
        for effect in causal_results.causal_effects[:5]:
            significance = "✅ Significant" if effect.is_significant else "❌ Not significant"
        ...
        print("This is expected with synthetic data. Use real data for meaningful results.")
...
def demonstrate_advanced_benchmarking():
    ...
    for i in range(20):
        viral_content = ContentData(
            ...
            text=f"VIRAL: This amazing discovery will blow your mind! 🤯 #viral #trending #mustread",
    ...
    try:
        ...
        if report.benchmark_comparison:
            print(f"Similarity to viral content: {report.benchmark_comparison.similarity_score:.2%}")
            
            if report.benchmark_comparison.improvement_suggestions:
                print("\n💡 Improvement Suggestions:")
                for suggestion in report.benchmark_comparison.improvement_suggestions[:3]:
                    print(f"• {suggestion}")
        ...
        print("This is expected with synthetic data. Use real viral examples for best results.")
...
def demonstrate_clustering():
    ...
    try:
        ...
        if clustering_results:
            ...
            if content_clusters:
                print(f"📊 Identified {content_clusters.get('n_clusters', 0)} content clusters")
                
                cluster_analysis = content_clusters.get('cluster_analysis', {})
                for cluster_name, info in cluster_analysis.items():
                    print(f"• {cluster_name}: {info['size']} items, {info['viral_rate']:.1%} viral rate")
        ...
        print("This is expected with synthetic data. Use real content data for meaningful clusters.")
