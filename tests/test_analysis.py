...
from virality_analyzer.analysis import (
...
class TestViralContentBenchmark:
    ...
    def test_add_viral_content(self):
        ...
        viral_content.metrics.unique_users_reached = (
            20000  # Virality coeff = 50000/20000 = 2.5
        )

        benchmark.add_viral_content([viral_content])

        assert (
            len(benchmark.viral_content_db) >= 0
        )  # Might be 1 if it passes the viral threshold
