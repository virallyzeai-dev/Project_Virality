...
async def health_check():
    ...
    return {
        "status": "healthy",
        "model_trained": diagnostic_engine is not None
        and diagnostic_engine.predictor.is_trained,
...
async def train_model(request: TrainingRequest):
    ...
    try:
        ...
        training_data = [
            api_content_to_core_content(content) for content in request.content_list
        ]
        ...
        return TrainingResponse(success=False, message=f"Training failed: {str(e)}")
...
async def start_batch_analysis(
    ...
    return BatchAnalysisResponse(job_id=job_id, status="started")
...
async def run_batch_analysis(
    ...
    try:
        ...
        core_content_list = [
            api_content_to_core_content(content) for content in content_list
        ]
...
async def add_viral_content(content_list: List[ContentDataAPI]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add benchmark content: {str(e)}"
        )
...
async def get_model_info():
    ...
    return {
        ...
        "feature_count": len(
            diagnostic_engine.predictor.feature_processor.feature_names
        ),
        "benchmark_examples": len(diagnostic_engine.benchmark.viral_content_db)
        if diagnostic_engine.benchmark
        else 0,
