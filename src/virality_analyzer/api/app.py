...
class ViralityAnalyzerApp:
    ...
    def run(self):
        ...
        page = st.sidebar.selectbox(
            ...
            ["Single Content Analysis", "Batch Analysis", "Training & Setup", "Analytics Dashboard"]
        )
    ...
    def single_content_page(self):
        ...
        if not self._is_model_ready():
            st.warning("⚠️ Please train the model first in the 'Training & Setup' page.")
            return
        ...
        with st.form("content_form"):
            ...
            with col1:
                content_id = st.text_input("Content ID", value=f"content_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
                platform = st.selectbox("Platform", [p.value for p in Platform])
                content_type = st.selectbox("Content Type", [c.value for c in ContentType])
                
            with col2:
                posting_time = st.datetime_input("Posting Time", datetime.now())
                hashtags = st.text_input("Hashtags (comma-separated)", placeholder="#viral, #trending")
                mentions = st.text_input("Mentions (comma-separated)", placeholder="@user1, @user2")
            
            # Content input
            text_content = st.text_area("Text Content", height=100, placeholder="Enter your content text here...")
            ...
            with metric_col4:
                ...
                completed_views = st.number_input("Completed Views", min_value=0, value=0)
    ...
    def batch_analysis_page(self):
        ...
        if uploaded_file is not None:
            ...
            required_cols = ['content_id', 'text', 'platform', 'likes', 'shares', 'comments', 'impressions']
    ...
    def training_setup_page(self):
        ...
        tab1, tab2, tab3 = st.tabs(["Model Training", "Viral Content Database", "Settings"])
        ...
        with tab1:
            ...
            if training_file is not None:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    model_type = st.selectbox("Model Type", ["xgboost", "catboost", "random_forest", "neural_net"])
                    virality_threshold = st.number_input("Virality Threshold", min_value=0.1, max_value=10.0, value=2.0)
                ...
                if st.button("🎯 Train Model"):
                    try:
                        ...
                        training_content_data = self._dataframe_to_content_data(training_df)
                        
                        with st.spinner("🔄 Training model..."):
                            # Train the model
                            self.diagnostic_engine.setup_predictor(training_content_data)
            ...
            if viral_file is not None:
                ...
                if st.button("📚 Setup Benchmark Database"):
                    if self._is_model_ready():
                        try:
                            viral_content_data = self._dataframe_to_content_data(viral_df)
                            
                            with st.spinner("🔄 Setting up benchmark..."):
                                self.diagnostic_engine.setup_benchmark(viral_content_data)
        ...
        with tab3:
            ...
            # API Keys
            openai_key = st.text_input("OpenAI API Key (optional)", type="password", help="For enhanced explanations")
            ...
            if self._is_model_ready():
                ...
                with model_info:
                    st.write(f"Model Type: {self.diagnostic_engine.predictor.model_type}")
                    st.write(f"Virality Threshold: {self.diagnostic_engine.virality_threshold}")
                    st.write(f"Feature Count: {len(self.diagnostic_engine.predictor.feature_processor.feature_names)}")
            else:
    ...
    def analytics_dashboard_page(self):
        ...
        if len(self.diagnostic_engine.analysis_history) >= 5:
            ...
            scores = [r.virality_prediction.virality_score for r in self.diagnostic_engine.analysis_history]
            dates = [r.generated_at for r in self.diagnostic_engine.analysis_history]
            
            trend_df = pd.DataFrame({
                'Date': dates,
                'Virality Score': scores
            })
    ...
    def _dataframe_to_content_data(self, df: pd.DataFrame) -> List[ContentData]:
        ...
        for _, row in df.iterrows():
            ...
            metrics = ContentMetrics(
                ...
                total_views=int(row.get('total_views', 0)) if pd.notna(row.get('total_views')) else None,
                completed_views=int(row.get('completed_views', 0)) if pd.notna(row.get('completed_views')) else None
            )
            ...
            content_data = ContentData(
                ...
                video_url=str(row.get('video_url')) if pd.notna(row.get('video_url')) else None,
    ...
    def _display_single_analysis_results(self, report):
        ...
        if report.benchmark_comparison:
            ...
            with col1:
                st.metric("Similarity to Viral Content", f"{benchmark.similarity_score:.2%}")
    ...
    def _display_batch_analysis_results(self, batch_results):
        ...
        if individual_reports:
            ...
            for report in individual_reports:
                ...
                results_data.append({
                    ...
                    'Viral Prediction': '✅ Viral' if pred.is_viral_prediction else '❌ Non-Viral',
                    'Virality Score': f"{pred.virality_score:.2%}",
                    'Confidence': f"{pred.confidence:.2%}",
                    'Top Factor': list(pred.feature_importance.keys())[0] if pred.feature_importance else 'N/A'
                })
        ...
        if patterns:
            ...
            if platform_perf:
                ...
                platform_df = pd.DataFrame([
                    {'Platform': k, 'Viral Rate': f"{v['viral_rate']:.2%}", 'Avg Score': f"{v['avg_score']:.2%}"}
        ...
        if clustering:
            ...
            if content_clusters:
                st.write(f"**Identified {content_clusters.get('n_clusters', 0)} content clusters**")
                
                cluster_analysis = content_clusters.get('cluster_analysis', {})
                for cluster_name, cluster_info in cluster_analysis.items():
                    with st.expander(f"{cluster_name.replace('_', ' ').title()} ({cluster_info['size']} items)"):
                        st.write(f"Viral Rate: {cluster_info['viral_rate']:.2%}")
                        st.write(f"Avg Virality: {cluster_info['avg_virality_coefficient']:.3f}")
                        
                        if cluster_info.get('top_distinctive_features'):
                            st.write("Top Distinctive Features:")
                            for feature, data in cluster_info['top_distinctive_features'][:3]:
                                st.write(f"• {feature}: {data['relative_difference']:+.1%} vs average")
