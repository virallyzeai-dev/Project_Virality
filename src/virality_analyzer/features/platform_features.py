...
from ..core.models import Platform, ContentData
...
class PlatformFeatureExtractor:
    ...
    def extract_timing_features(
        ...
        if peak_hours:
            distances = [min(abs(posting_hour - ph), 24 - abs(posting_hour - ph)) for ph in peak_hours]
    ...
    def extract_hashtag_features(
        ...
        optimal_hashtag_count = 1.0 if hashtag_count == optimal_count else max(0.0, 1.0 - abs(hashtag_count - optimal_count) / optimal_count)
    ...
    def extract_algorithm_features(
        ...
        if content_data.text:
            ...
            content_length_optimal = max(0.0, 1.0 - abs(text_length - optimal_length) / optimal_length)
    ...
    def extract_competition_features(
        ...
        return {
            ...
            "content_saturation": competition_level * 0.8  # Proxy for content saturation
        }
    
    def update_trending_data(self, trending_hashtags: Set[str], hashtag_scores: Dict[str, float]) -> None:
    ...
    def extract_all_features(self, content_data: ContentData) -> Dict[str, float]:
        ...
        if content_data.posting_time:
            features.update(self.extract_timing_features(
                content_data.posting_time, 
        ...
        # Algorithm features
        features.update(self.extract_algorithm_features(
            content_data, 
            content_data.platform
        ))
        ...
        if content_data.posting_time:
            features.update(self.extract_competition_features(
                content_data.posting_time, 
                content_data.platform
            ))
