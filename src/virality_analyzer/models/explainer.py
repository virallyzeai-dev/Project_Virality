...
class ModelExplainer:
    ...
    def explain_with_lime(
        ...
        try:
            ...
            for feature_idx, importance in explanation.as_list():
                feature_name = self.predictor.feature_processor.feature_names[feature_idx]
    ...
    def generate_text_explanation(
        ...
        if feature_importance:
            ...
            if positive_factors:
                top_positive = sorted(positive_factors.items(), key=lambda x: x[1], reverse=True)[:3]
                factor_names = [self._humanize_feature_name(name) for name, _ in top_positive]
                explanation_parts.append(f"Key factors supporting virality: {', '.join(factor_names)}")
            
            if negative_factors:
                top_negative = sorted(negative_factors.items(), key=lambda x: abs(x[1]), reverse=True)[:3]
                factor_names = [self._humanize_feature_name(name) for name, _ in top_negative]
                explanation_parts.append(f"Key factors hindering virality: {', '.join(factor_names)}")
    ...
    def generate_llm_explanation(
        ...
        try:
            ...
            context = self._prepare_llm_context(content_data, prediction_result, feature_importance)
    ...
    def _prepare_llm_context(
        ...
        if content_data.text:
            context_parts.append(f"Content text: '{content_data.text[:200]}...' if longer")
        ...
        # Prediction
        context_parts.append(f"Virality prediction: {prediction_result.virality_score:.1%} probability")
        context_parts.append(f"Predicted as: {'VIRAL' if prediction_result.is_viral_prediction else 'NON-VIRAL'}")
        
        # Key features
        if feature_importance:
            top_features = sorted(feature_importance.items(), key=lambda x: abs(x[1]), reverse=True)[:5]
            feature_list = [f"{self._humanize_feature_name(name)}: {value:.3f}" for name, value in top_features]
            context_parts.append(f"Key influencing factors: {'; '.join(feature_list)}")
        
        context_parts.append("Please explain in simple terms why this content is predicted to be viral or not viral, and what could be improved.")
    ...
    def _get_factor_recommendations(self, factor: str, impact: float) -> List[str]:
        ...
        factor_recommendations = {
            'sentiment_polarity': ["Consider using more positive language", "Add emotional appeal to your content"],
            'emotional_intensity': ["Increase emotional engagement", "Use more compelling storytelling"],
            'readability_score': ["Simplify your language for broader appeal", "Use shorter sentences"],
            'hashtag_count': ["Optimize your hashtag usage", "Research trending hashtags"],
            'posting_hour': ["Post during peak engagement hours", "Consider your audience's time zone"],
            'text_length': ["Adjust content length for your platform", "Consider breaking up long content"],
            'face_count': ["Include people in your visuals", "Use authentic human faces"],
            'brightness': ["Improve image lighting and contrast", "Use more vibrant visuals"],
            'aesthetic_score': ["Enhance visual quality", "Use professional photography/editing"]
        }
    ...
    def _get_platform_recommendations(self, content_data: ContentData) -> List[str]:
        ...
        platform_recs = {
            'twitter': ["Keep text under 280 characters", "Use 1-2 relevant hashtags", "Include visuals"],
            'instagram': ["Use high-quality visuals", "Write engaging captions", "Use 5-11 hashtags"],
            'tiktok': ["Create engaging first 3 seconds", "Use trending sounds", "Keep videos under 60 seconds"],
            'youtube': ["Create compelling thumbnails", "Write descriptive titles", "Optimize for search"],
            'linkedin': ["Share professional insights", "Use industry-relevant hashtags", "Engage in discussions"]
        }
        
        return platform_recs.get(content_data.platform.value, ["Focus on high-quality content", "Engage with your audience"])
