...
class FeatureProcessor:
    ...
    def extract_features(self, content_data: ContentData) -> FeatureVector:
        ...
        if content_data.text:
            feature_vector.text_features = self.text_extractor.extract_all_features(
                content_data.text
            )
            feature_vector.embeddings = self.text_extractor.get_text_embeddings(
                content_data.text
            )

        # Visual features
        if content_data.image_url:
            feature_vector.visual_features = self.visual_extractor.extract_all_features(
                content_data.image_url
            )
        elif content_data.video_url:
            feature_vector.visual_features = (
                self.visual_extractor.extract_video_features(content_data.video_url)
            )

        # Engagement features
        feature_vector.engagement_features = (
            self.engagement_extractor.extract_all_features(
                content_data.metrics, content_data.posting_time
            )
        )

        # Platform features
        feature_vector.platform_features = self.platform_extractor.extract_all_features(
            content_data
        )
