...
try:
    ...
    print("Warning: DoWhy not available. Some causal analysis features will be limited.")
...
try:
    ...
    print("Warning: CausalML not available. Some causal analysis features will be limited.")

from sklearn.ensemble import RandomForestRegressor
from sklearn.linear_model import LinearRegression
from scipy import stats

from ..core.models import ContentData, CausalEffect, CausalAnalysisResult
...
class CausalAnalyzer:
    ...
    def prepare_causal_data(self, content_data_list: List[ContentData]) -> pd.DataFrame:
        ...
        for content_data in content_data_list:
            ...
            row['virality_coefficient'] = self.feature_processor.engagement_extractor.metrics_calculator.calculate_virality_coefficient(
                content_data.metrics.shares,
                content_data.metrics.unique_users_reached or content_data.metrics.impressions
            )
            row['is_viral'] = 1 if row['virality_coefficient'] >= self.virality_threshold else 0
    ...
    def identify_confounders(self, data: pd.DataFrame, treatment: str, outcome: str) -> List[str]:
    ...
    def analyze_feature_causality(
        ...
        for feature in available_features:
            if data[feature].nunique() > 1:  # Skip constant features
                ...
                pooled_std = np.sqrt(((len(treated) - 1) * treated.var() + 
                ...
                if DOWHY_AVAILABLE and len(confounders) > 0:
                    dowhy_results = self.dowhy_causal_analysis(data, feature, outcome, confounders)
                    if dowhy_results:
        ...
        causal_effects.sort(key=lambda x: (x.is_significant, abs(x.effect_size)), reverse=True)
        
        return CausalAnalysisResult(
            content_id="batch_analysis",
            causal_effects=causal_effects,
            confounders=self.identify_confounders(data, 'sentiment_polarity', outcome)  # Example
        )
    ...
    def analyze_treatment_heterogeneity(
        ...
        for segment_feature in segment_features:
            if segment_feature in data.columns:
                ...
                high_segment = data[data[segment_feature] > data[segment_feature].median()]
                low_segment = data[data[segment_feature] <= data[segment_feature].median()]
                ...
                if len(high_segment) > 10 and len(low_segment) > 10:
                    ...
                    heterogeneity_results[segment_feature] = {
                        ...
                        'difference': high_effect['simple_effect'] - low_effect['simple_effect']
                    }
