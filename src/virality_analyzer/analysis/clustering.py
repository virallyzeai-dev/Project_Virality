...
class ContentClusterAnalyzer:
    ...
    def prepare_clustering_data(
        self, content_data_list: List[ContentData]
    ) -> pd.DataFrame:
    ...
    def find_optimal_clusters(
        self, data: pd.DataFrame, max_clusters: int = 10, method: str = "kmeans"
    ) -> int:
    ...
    def _analyze_clusters(
        self, data: pd.DataFrame, cluster_labels: np.ndarray, method: str
    ) -> Dict[str, Any]:
        ...
        for cluster_id in unique_clusters:
            cluster_data = data_with_clusters[
                data_with_clusters["cluster"] == cluster_id
            ]
            ...
            cluster_info = {
                ...
                "platform_distribution": cluster_data["platform"]
                .value_counts()
                .to_dict(),
                "content_type_distribution": cluster_data["content_type"]
                .value_counts()
                .to_dict(),
            }
            ...
            feature_cols = [
                ...
                if col
                not in ["content_id", "is_viral", "virality_coefficient", "cluster"]
            ]
    ...
    def cluster_audience_behavior(
            ...
            if metrics.impressions > 0:
                row = {
                    ...
                    "virality_coefficient": self.feature_processor.engagement_extractor.metrics_calculator.calculate_virality_coefficient(
                        metrics.shares,
                        metrics.unique_users_reached or metrics.impressions,
                    ),
        ...
        for cluster_id in range(optimal_k):
            cluster_data = engagement_df[
                engagement_df["behavior_cluster"] == cluster_id
            ]
        ...
        for cluster_name, pattern in behavior_patterns.items():
            if "High viral potential" in pattern["description"]:
                recommendations.append(
                    f"Target {cluster_name}: Focus on shareable content with emotional appeal"
                )
            elif "Share-focused" in pattern["description"]:
                recommendations.append(
                    f"Target {cluster_name}: Create content that encourages sharing"
                )
            elif "Discussion-focused" in pattern["description"]:
                recommendations.append(
                    f"Target {cluster_name}: Ask questions and encourage comments"
                )
            elif "Appreciation-focused" in pattern["description"]:
                recommendations.append(
                    f"Target {cluster_name}: Focus on quality over viral potential"
                )

        return recommendations

    def visualize_clusters(
        self, method: str = "kmeans", save_path: Optional[str] = None
    ) -> None:
    ...
    def get_cluster_insights(self, method: str = "kmeans") -> Dict[str, str]:
        ...
        for cluster_id in unique_clusters:
            ...
            top_platform = (
                cluster_data["platform"].mode().iloc[0]
                if not cluster_data["platform"].mode().empty
                else "Mixed"
            )
            top_type = (
                cluster_data["content_type"].mode().iloc[0]
                if not cluster_data["content_type"].mode().empty
                else "Mixed"
            )
