...
class DiagnosticEngine:
    ...
    def setup_predictor(self, training_data: List[ContentData]) -> None:
        ...
        print(
            f"Training completed with F1 score: {training_result.performance.f1_score:.3f}"
        )
        ...
        background_data = self.predictor.feature_processor.transform_features(
            feature_vectors
        )
    ...
    def setup_benchmark(self, viral_content_data: List[ContentData]) -> None:
        ...
        print(
            f"Added {len(self.benchmark.viral_content_db)} viral examples to benchmark"
        )
    ...
    def diagnose_batch_content(
        ...
        if include_clustering:
            ...
            behavior_results = self.cluster_analyzer.cluster_audience_behavior(
                content_data_list
            )
        ...
        if include_causal:
            ...
            causal_results = self.causal_analyzer.analyze_feature_causality(
                content_data_list
            )
            batch_results["causal_analysis"] = causal_results

        # Pattern analysis
        batch_results["patterns"] = self._analyze_content_patterns(
            content_data_list, individual_reports
        )

        # Overall recommendations
        batch_results["recommendations"] = self._generate_batch_recommendations(
            batch_results
        )
    ...
    def _calculate_summary_stats(
        ...
        if benchmark_scores:
            ...
            summary["high_similarity_count"] = sum(
                1 for score in benchmark_scores if score > 0.8
            )

        return summary

    def _analyze_content_patterns(
        self, content_data_list: List[ContentData], reports: List[DiagnosticReport]
    ) -> Dict[str, Any]:
        ...
        for content_data, report in zip(content_data_list, reports):
            ...
            platform_stats[platform]["scores"].append(
                report.virality_prediction.virality_score
            )
        ...
        for content_data, report in zip(content_data_list, reports):
            ...
            type_stats[content_type]["scores"].append(
                report.virality_prediction.virality_score
            )
            if report.virality_prediction.is_viral_prediction:
        ...
        if non_viral_factors:
            non_viral_factor_means = {
                k: np.mean(v) for k, v in non_viral_factors.items()
            }
            patterns["common_failure_factors"] = dict(
                sorted(
                    non_viral_factor_means.items(),
    ...
    def _generate_batch_recommendations(
        ...
        if viral_rate < 0.1:
            recommendations.append(
                "Overall viral potential is low. Focus on improving content quality and engagement factors."
            )
        elif viral_rate > 0.5:
            recommendations.append(
                "Strong viral potential detected. Maintain current content strategy while optimizing timing and distribution."
            )
        ...
        if platform_perf:
            ...
            worst_platform = min(
                platform_perf.items(), key=lambda x: x[1]["viral_rate"]
            )

            recommendations.append(
                f"Best performing platform: {best_platform[0]} ({best_platform[1]['viral_rate']:.1%} viral rate)"
            )
            if worst_platform[1]["viral_rate"] < 0.1:
                recommendations.append(
                    f"Consider strategy adjustment for {worst_platform[0]} (only {worst_platform[1]['viral_rate']:.1%} viral rate)"
                )
        ...
        if success_factors:
            ...
            recommendations.append(
                f"Key success factor identified: {top_factor}. Focus on optimizing this across all content."
            )
        ...
        if clustering and "behavior_clusters" in clustering:
            ...
            recommendations.extend(
                behavior_recs[:2]
            )  # Add top 2 behavior recommendations
    ...
    def generate_report_summary(self, report: DiagnosticReport) -> str:
        ...
        if report.benchmark_comparison:
            ...
            summary_parts.append(f"Similarity to viral content: {similarity:.1%}")

            if similarity < 0.5:
                summary_parts.append(
                    "Content differs significantly from successful viral examples."
                )
    ...
    def get_analysis_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "avg_virality_score": np.mean(
                [r.virality_prediction.virality_score for r in self.analysis_history]
            ),
            "viral_prediction_rate": np.mean(
                [
                    r.virality_prediction.is_viral_prediction
    ...
    def _get_most_common_factors(self) -> Dict[str, float]:
        ...
        for report in self.analysis_history:
            for (
                factor,
                importance,
            ) in report.virality_prediction.feature_importance.items():
                if factor not in all_factors:
                    all_factors[factor] = []
        ...
        return dict(
            sorted(factor_averages.items(), key=lambda x: x[1], reverse=True)[:10]
        )
